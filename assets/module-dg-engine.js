/* ===========================================================
   CAUGIA DG MODULE ENGINE v1.0 (DETERMINISTIC + SCORES)
   Based on Intelligence Engine v9.0
   Adapted for: Demand Generation Deep Dive (60 questions, 6 engines)
   
   Key differences from v9:
   - Storage key: caugia_dg_v1_state
...
(function () {
  ...
  // --- 1. CONFIGURATION ---
  const CONFIG = {
    webhookUrl: "https://hook.eu1.make.com/col4r5kaw5ljv4e3uus1qngijzet8kh5",
    storageKey: "caugia_dg_v1_state",
    autoSaveInterval: 1000,
    schemaVersion: "dg-1.0"
  };
  ...
  function init() {
    if (!window.QUESTIONS || !Array.isArray(window.QUESTIONS) || window.QUESTIONS.length === 0) {
      console.error("❌ DG_QUESTIONS.js not loaded or empty.");
    ...
    console.log("DG Engine v" + CONFIG.schemaVersion + " started. " + window.QUESTIONS.length + " questions.");
    renderQuestion();
    updateSidebar();
  ...
  async function submitData(isTest) {
    ...
    var message = isTest ? "DG Test Submission" : "DG Official Submission";
    ...
    console.log("🚀 DG Payload:", payload);
    ...
    try {
      ...
      if (isTest) {
        var lines = [];
        lines.push("DG Overall: " + overallScore);
        for (var e = 1; e <= 6; e++) {
        ...
        alert("✅ DG TEST SUCCESS!\n\n" + lines.join("\n"));
      } else {
        STATE.completed = true;
        localStorage.removeItem(CONFIG.storageKey);
        window.location.href = "module-dg-thank-you.html";
      }
    } catch (e) {
  ...
  function resetAll() {
    if (!confirm("Reset all DG answers?")) return;
  ...
  if (UI.testBtn) {
    console.log("✅ DG Test Button Bound");
