/* ===========================================================
   CAUGIA CSE MODULE ENGINE v1.0 (DETERMINISTIC + SCORES)
   Based on Intelligence Engine v9.0
   Adapted for: Customer Success & Expansion Deep Dive (60 questions, 6 engines)
   
   Key differences from v9:
   - Storage key: caugia_cse_v1_state
...
(function () {
  ...
  // --- 1. CONFIGURATION ---
  const CONFIG = {
    webhookUrl: "https://hook.eu1.make.com/fsb67pt3of4p3s6rxehlbv2hflzp9op4",
    storageKey: "caugia_cse_v1_state",
    autoSaveInterval: 1000,
    schemaVersion: "cse-1.0"
  };
  ...
  function init() {
    if (!window.QUESTIONS || !Array.isArray(window.QUESTIONS) || window.QUESTIONS.length === 0) {
      console.error("❌ CSE_QUESTIONS.js not loaded or empty.");
    ...
    console.log("CSE Engine v" + CONFIG.schemaVersion + " started. " + window.QUESTIONS.length + " questions.");
    renderQuestion();
    updateSidebar();
  ...
  async function submitData(isTest) {
    ...
    var message = isTest ? "CSE Test Submission" : "CSE Official Submission";
    ...
    var payload = {
      metadata: {
        ...
        module: "cse",
        module_name: "Customer Success & Expansion Deep Dive",
        questions_count: window.QUESTIONS.length,
        source: "CSE Engine v" + CONFIG.schemaVersion,
        is_test: !!isTest
      },
      message: message,
    ...
    console.log("🚀 CSE Payload:", payload);
    ...
    try {
      ...
      if (isTest) {
        var lines = [];
        lines.push("CSE Overall: " + overallScore);
        for (var e = 1; e <= 6; e++) {
        ...
        alert("✅ CSE TEST SUCCESS!\n\n" + lines.join("\n"));
      } else {
        STATE.completed = true;
        localStorage.removeItem(CONFIG.storageKey);
        window.location.href = "module-cse-thank-you.html";
      }
    } catch (e) {
  ...
  function resetAll() {
    if (!confirm("Reset all CSE answers?")) return;
  ...
  if (UI.testBtn) {
    console.log("✅ CSE Test Button Bound");
