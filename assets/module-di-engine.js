/* ===========================================================
   CAUGIA DI MODULE ENGINE v1.0 (DETERMINISTIC + SCORES)
   Based on Intelligence Engine v9.0
   Adapted for: Data & Insights Deep Dive (60 questions, 6 engines)
   
   Key differences from v9:
   - Storage key: caugia_di_v1_state
...
(function () {
  ...
  // --- 1. CONFIGURATION ---
  const CONFIG = {
    webhookUrl: "https://hook.eu1.make.com/oon39odbkw0m0v1k2g422tivx6dnxv1y",
    storageKey: "caugia_di_v1_state",
    autoSaveInterval: 1000,
    schemaVersion: "di-1.0"
  };
  ...
  function init() {
    if (!window.QUESTIONS || !Array.isArray(window.QUESTIONS) || window.QUESTIONS.length === 0) {
      console.error("❌ DI_QUESTIONS.js not loaded or empty.");
    ...
    console.log("DI Engine v" + CONFIG.schemaVersion + " started. " + window.QUESTIONS.length + " questions.");
    renderQuestion();
    updateSidebar();
  ...
  async function submitData(isTest) {
    ...
    var message = isTest ? "DI Test Submission" : "DI Official Submission";
    ...
    var payload = {
      metadata: {
        ...
        module: "di",
        module_name: "Data & Insights Deep Dive",
    ...
    console.log("🚀 DI Payload:", payload);
    ...
    try {
      ...
      if (isTest) {
        var lines = [];
        lines.push("DI Overall: " + overallScore);
        for (var e = 1; e <= 6; e++) {
  ...
  function resetAll() {
    if (!confirm("Reset all DI answers?")) return;
  ...
  if (UI.testBtn) {
    console.log("✅ DI Test Button Bound");
