/* ===========================================================
   CAUGIA EN MODULE ENGINE v1.0 (DETERMINISTIC + SCORES)
   Based on Intelligence Engine v9.0
   Adapted for: Enablement Deep Dive (60 questions, 6 engines)
   
   Key differences from v9:
   - Storage key: caugia_en_v1_state
...
(function () {
  ...
  // --- 1. CONFIGURATION ---
  const CONFIG = {
    webhookUrl: "https://hook.eu1.make.com/iznspp3h6jf0wbufph4o56tmnejtkpvu",
    storageKey: "caugia_en_v1_state",
    autoSaveInterval: 1000,
    schemaVersion: "en-1.0"
  };
  ...
  function init() {
    if (!window.QUESTIONS || !Array.isArray(window.QUESTIONS) || window.QUESTIONS.length === 0) {
      console.error("❌ EN_QUESTIONS.js not loaded or empty.");
    ...
    console.log("EN Engine v" + CONFIG.schemaVersion + " started. " + window.QUESTIONS.length + " questions.");
    renderQuestion();
    updateSidebar();
  ...
  async function submitData(isTest) {
    ...
    var message = isTest ? "EN Test Submission" : "EN Official Submission";
    ...
    var payload = {
      metadata: {
        ...
        module: "en",
        module_name: "Enablement Deep Dive",
        questions_count: window.QUESTIONS.length,
        source: "EN Engine v" + CONFIG.schemaVersion,
        is_test: !!isTest
      },
      message: message,
    ...
    console.log("🚀 EN Payload:", payload);
    ...
    try {
      ...
      if (isTest) {
        var lines = [];
        lines.push("EN Overall: " + overallScore);
        for (var e = 1; e <= 6; e++) {
        ...
        alert("✅ EN TEST SUCCESS!\n\n" + lines.join("\n"));
      } else {
        STATE.completed = true;
        localStorage.removeItem(CONFIG.storageKey);
        window.location.href = "module-en-thank-you.html";
      }
    } catch (e) {
  ...
  function resetAll() {
    if (!confirm("Reset all EN answers?")) return;
  ...
  if (UI.testBtn) {
    console.log("✅ EN Test Button Bound");
