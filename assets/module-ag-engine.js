/* ===========================================================
   CAUGIA AG MODULE ENGINE v1.0 (DETERMINISTIC + SCORES)
   Based on Intelligence Engine v9.0
   Adapted for: Alignment & Governance Deep Dive (60 questions, 6 engines)
   
   Key differences from v9:
   - Storage key: caugia_ag_v1_state
...
(function () {
  ...
  // --- 1. CONFIGURATION ---
  const CONFIG = {
    webhookUrl: "https://hook.eu1.make.com/qab4wjm2xtuu4nmob3245l7w55v8d1sd",
    storageKey: "caugia_ag_v1_state",
    autoSaveInterval: 1000,
    schemaVersion: "ag-1.0"
  };
  ...
  function init() {
    if (!window.QUESTIONS || !Array.isArray(window.QUESTIONS) || window.QUESTIONS.length === 0) {
      console.error("❌ AG_QUESTIONS.js not loaded or empty.");
    ...
    console.log("AG Engine v" + CONFIG.schemaVersion + " started. " + window.QUESTIONS.length + " questions.");
    renderQuestion();
    updateSidebar();
  ...
  async function submitData(isTest) {
    ...
    var message = isTest ? "AG Test Submission" : "AG Official Submission";
    ...
    console.log("🚀 AG Payload:", payload);
    ...
    try {
      ...
      if (isTest) {
        var lines = [];
        lines.push("AG Overall: " + overallScore);
        for (var e = 1; e <= 6; e++) {
        ...
        alert("✅ AG TEST SUCCESS!\n\n" + lines.join("\n"));
      } else {
        STATE.completed = true;
        localStorage.removeItem(CONFIG.storageKey);
        window.location.href = "module-ag-thank-you.html";
      }
    } catch (e) {
  ...
  function resetAll() {
    if (!confirm("Reset all AG answers?")) return;
  ...
  if (UI.testBtn) {
    console.log("✅ AG Test Button Bound");
